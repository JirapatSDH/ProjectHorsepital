...
namespace Project1
{
    public class Room1 : Screen
    {
        ...
        public Room1 (Game1 game, EventHandler theScreenEvent) : base(theScreenEvent)
        {
            this.game = game;

            bGtile = game.Content.Load<Texture2D>("1D");
            ...
            pos = new Vector2(500, 270);
            ballPos = new Vector2(650, 255);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            KeyboardState old_ks = Keyboard.GetState();
            {
                ...
                if (ks.IsKeyDown(Keys.D) && pos.X < game.GraphicsDevice.Viewport.Width - 100)
                {
                    if (pos.X >= fLine.X && camPos.X < game.GraphicsDevice.Viewport.Width)
                    {
                        fLine += new Vector2(3, 0);
                        bLine += new Vector2(3, 0);
                        camPos += new Vector2(3, 0);
                ...
                if (personRectangle.Intersects(ballRectangle) == true)
                {
                    ...
                    text = "F To Enter";
                    {
                        if (ks.IsKeyDown(Keys.F) == true && ks.IsKeyUp(Keys.F) == false) //Intereact object
                        {
                            pos.X = 500;
                            pos.Y = 270;
                            ScreenEvent.Invoke(game.mMain_HallScreen, new EventArgs());
                            return;
                        }
